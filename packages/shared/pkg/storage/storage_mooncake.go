...
func (b *mooncakeBlob) Put(ctx context.Context, data []byte) error {
...
func (s *mooncakeSeekable) ReadAt(ctx context.Context, buffer []byte, off int64) (int, error) {
	if len(buffer) == 0 {
...
func (s *mooncakeSeekable) OpenRangeReader(ctx context.Context, off, length int64) (io.ReadCloser, error) {
	if length < 0 {
		return nil, fmt.Errorf("length must be non-negative")
	}

	data := make([]byte, length)
	n, err := s.ReadAt(ctx, data, off)
...
func (s *mooncakeSeekable) StoreFile(ctx context.Context, filePath string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal mooncake manifest: %w", err)
	}

	if err := s.client.Put(ctx, s.manifestKey(), manifestData); err != nil {
		return fmt.Errorf("failed to write mooncake manifest: %w", err)
	}
