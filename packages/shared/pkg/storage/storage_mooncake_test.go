...
func TestMooncakeSeekableStoresManifestAndBlocks(t *testing.T) {
	...
	seekable, err := provider.OpenSeekable(t.Context(), "build/rootfs.ext4", RootFSObjectType)
	require.NoError(t, err)

	require.NoError(t, seekable.StoreFile(t.Context(), srcPath))
	...
	buf := make([]byte, 7)
	n, err := seekable.ReadAt(t.Context(), buf, 3)
	require.NoError(t, err)
	assert.Equal(t, 7, n)
...
func TestMooncakeSeekableReadAtEOF(t *testing.T) {
	...
	seekable, err := provider.OpenSeekable(t.Context(), "build/memfile", MemfileObjectType)
	require.NoError(t, err)
	require.NoError(t, seekable.StoreFile(t.Context(), srcPath))

	buf := make([]byte, 4)
	n, err := seekable.ReadAt(t.Context(), buf, 4)
...
func TestMooncakeSeekableOpenRangeReader(t *testing.T) {
	...
	reader, err := seekable.OpenRangeReader(t.Context(), 2, 8)
	require.NoError(t, err)
	defer reader.Close()
